...
class BTTraceHelper:
    ...
    def write_to_file(self, path: Path, content: str, mode: str = "w") -> bool:
        ...
        try:
            ...
            print(
                f"Error: Permission denied writing to {path}. Run with sudo.",
                file=sys.stderr,
            )
            return False
    ...
    def setup_tracing(self) -> bool:
        ...
        if not self.check_root():
            print(
                "Error: Root privileges required for tracing setup. Run with sudo.",
                file=sys.stderr,
            )
            return False
        ...
        for func in self.TRACE_FUNCTIONS:
            if not self.write_to_file(
                self.TRACING_PATH / "set_ftrace_filter", f"{func}\n", mode="a"
            ):
                print(
                    f"Warning: Failed to add filter for {func}",
                    file=sys.stderr,
                )

        # Enable function-fork option
        if not self.write_to_file(
            self.TRACING_PATH / "options/function-fork", "1"
        ):
            return False

        # Set tracer to function
        if not self.write_to_file(
            self.TRACING_PATH / "current_tracer", "function"
        ):
            return False
    ...
    def setup_dynamic_debug(self) -> bool:
        ...
        if not self.check_root():
            print(
                "Error: Root privileges required for dynamic debug setup. Run with sudo.",
                file=sys.stderr,
            )
            return False
        ...
        debug_filters = ["module bluetooth +p", "module btusb +p"]

        for filter in debug_filters:
            if not self.write_to_file(
                self.DYNAMIC_DEBUG_PATH, f"{filter}\n", mode="w"
            ):
                print(
                    f"Warning: Failed to set dynamic debug filter: {filter}",
                    file=sys.stderr,
                )
    ...
    def stop_dynamic_debug(self) -> bool:
        ...
        if not self.check_root():
            print(
                "Error: Root privileges required for dynamic debug cleanup. Run with sudo.",
                file=sys.stderr,
            )
            return False
        ...
        for filter in debug_filters:
            if not self.write_to_file(
                ...
                print(
                    f"Warning: Failed to clear dynamic debug filter: {filter}",
                    file=sys.stderr,
                )
    ...
    def start_log_collection(self, output_dir: Optional[str] = None) -> bool:
        ...
        log_configs = [
            {
                ...
                "output": _output_dir
                / (self.creation_string + "journal-kernel.log"),
                "needs_sudo": False,
            },
            {
                ...
                "output": _output_dir
                / (self.creation_string + "journal-bluetoothd.log"),
                "needs_sudo": False,
            },
            {
        ...
        for config in log_configs:
            if config["needs_sudo"] and not self.check_root():
                print(
                    f"Warning: Skipping {config['name']} (requires root)",
                    file=sys.stderr,
                )
                continue
        ...
        if not self.processes:
            print(
                "Error: No log collection processes started", file=sys.stderr
            )
            return False
            ...
            while self.running and any(
                ...
                for process in self.processes:
                    if process.poll() is not None:
                        print(
                            f"Warning: Process {process.pid} exited unexpectedly"
                        )
